...
export default function CashFlowReport({ data }: CashFlowReportProps) {
  ...
  const renderRow = (row: ReportRow, level = 0, parentIdx: string | number=0) => {
    if (row.RowType === "Section") {
      return (
        <React.Fragment key={`${row.Title}-${level}-${parentIdx}`}>
          <tr className="cf-section">
            <td colSpan={headers?.length || 2} style={{ paddingLeft: `${level * 20}px`, fontWeight: 'bold' }}>
              {row.Title}
            </td>
          </tr>
          {row.Rows?.map((subRow, subIdx) => renderRow(subRow, level + 1, `${parentIdx}-${level}`)).filter(Boolean)}
        </React.Fragment>
      );
    }
    ...
    if (row.RowType === "Row" || row.RowType === "SummaryRow") {
      return (
        <tr key={`${row.Title || 'row'}-${level}-${parentIdx}`} className={row.RowType === "SummaryRow" ? "cf-summary" : ""}>
          {row.Cells?.map((cell, idx) => (
            <td key={idx} className="cf-cell">{cell.Value}</td>
          ))}
        </tr>
      );
    }
